...
class ComposeRememberContentMissingCheckTest {
    ...
    private val rule = ComposeRememberContentMissingCheck(Config.empty)
    ...
    fun `errors when a non-remembered movableContentOf is used in a Composable`() {
        ...
        for (error in errors) {
            assertThat(error).hasMessage(ComposeRememberContentMissing.MovableContentOfNotRemembered)
        }
    }
    ...
    fun `errors when a non-remembered movableContentWithReceiverOf is used in a Composable`() {
        ...
        for (error in errors) {
            assertThat(error).hasMessage(ComposeRememberContentMissing.MovableContentWithReceiverOfNotRemembered)
        }
    }
}
